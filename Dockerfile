...
# and `pip install ai-lab` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "ai-lab", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n ai-lab -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "ai-lab", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "ai-lab", "YOUR_CONSOLE_SCRIPT"]
