...
# and `pip install ai_labs` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "ai_labs", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n ai_labs -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "ai_labs", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "ai_labs", "YOUR_CONSOLE_SCRIPT"]
